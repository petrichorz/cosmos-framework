...
| `joint_attn_implementation`    | `"two_way"`                                                     | VFM attention topology: `"two_way"` (U/G blocks with cross-attention), `"three_way"` (adds the sparsity-aware NATTEN path), or `"teacher_forcing"` (causal UND plus explicitly masked CLEAN/NOISY streams during training and two-way inference). **VFM only.** |
