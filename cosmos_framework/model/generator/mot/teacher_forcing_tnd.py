...
def build_tnd_plan(
    ...
    *,
    ...
    """
    if not 1 <= max_kv_tokens <= 1048576:
        raise ValueError("max_kv_tokens must be in [1, 1048576]")
    ...
    for sample, (length, history) in enumerate(zip(layout.sample_lens, layout.geometry.history_blocks, strict=True)):
        und = layout.split_lens[2 * sample]
        vision = layout.split_lens[2 * sample + 1] // 2
        clean_start = koffset + und
        sample_blocks = blocks[clean_start : clean_start + vision]
        starts = [0] + [i for i in range(1, vision) if sample_blocks[i] != sample_blocks[i - 1]] + [vision]
        for stream in (0, 1):
            for block, (start, end) in enumerate(zip(starts[:-1], starts[1:], strict=True)):
                history_start = starts[max(0, block - history)]
                group_k = list(range(koffset, clean_start))
                group_k.extend(range(clean_start + history_start, clean_start + start))
                group_k.extend(range(clean_start + stream * vision + start, clean_start + stream * vision + end))
                if len(group_k) > 1048576:
                    raise ValueError("one teacher-forcing KV group exceeds the TND token limit")
                if qe and (len(ks) + len(group_k) > max_kv_tokens or len(qe) == 1024):
                    flush()
