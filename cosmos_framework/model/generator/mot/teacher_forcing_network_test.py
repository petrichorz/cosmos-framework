...
def test_network_config_keeps_dense_mask_limit_explicit():
    config = Cosmos3VFMNetworkConfig(teacher_forcing_max_mask_elements=123)

    assert config.teacher_forcing_max_mask_elements == 123
    assert Cosmos3VFMNetworkConfig().teacher_forcing_max_mask_elements is None

    with pytest.raises(ValueError, match="teacher_forcing_max_mask_elements"):
        Cosmos3VFMNetworkConfig(teacher_forcing_max_mask_elements=0)
